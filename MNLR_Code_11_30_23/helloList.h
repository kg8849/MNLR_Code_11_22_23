...
checkIfSubstring is used in forward algorithm, the print statements are useful for forward algorithm analysis*/
 boolean checkIfSubstr(char* add1 , char* add2){
  ...
  /* check if a node's tier address qualifes it to be a grandparent from whom this node received a tier address*/
...
int delete() {
	...
	while (temp != NULL) {
		...
		if (delTimeDiff >= 3) { //dead timer(from 1.3 to 0.40) reduced 4/20/2022 - Samruddhi
