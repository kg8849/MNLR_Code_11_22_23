...
//FILE* fptr; //global variable. TO enable logs to be written to files - DO WE NEED THIS ANY MORE. WE ARE USING SCREEN COMMANDS 

int myTierValue = -1;
int tierAddrCount = 0;
int enableLogScreen = 1; //flag to set whether logs need to be shown in screen
//int enableLogFiles = 0; //flag to set whether logs need to be shown in file - NS - set this to 0 on 3-28-20 DO WE NEED THIS ANY MORE. WE ARE USING SCREEN COMMANDS 
...
// void signal_callbackZ_handler(int signum); // Function just to print the details based on the signal. NOT BEING USED 
// char* macAddrtoString(unsigned char* addr, char* strMAC, size_t size); NOT BEING USED 
// int trimAndDelNewLine(); NOT BEING USED 
// char* strrmc(char* str, char ch); NOT BEING USED 
// int packetStats(); NOT BEING USED 
// void printInputStats(); NOT BEING USED 
int setInterfaces();
int freeInterfaces();
...
// removed the old logic on 11/16/23. Sanrudhi's old code will still have it
	return updateStatus;
}
...
int _get_MACTest(struct addr_tuple* myAddr) {
	...
	// Creating the EIBP CONTROL SOCKET HERE
	...
	// Creating the EIBP IP SOCKET HERE
   ...
   **************************/ 
	...
	while (1) {
		...
		//Send hello packets after every 0.1 seconds // Pending - No need to send all the addresses evry time, need to work on that - 07/02/2021
		if (time01 - time00 > 1) {  //Hello timer (from 1.2 to 0.25 to 0.1) and dead timer(from 1.3 to 0.4 to 0.15) reduced 4/20/2022 - Samruddhi, Kct
		...
		if (flagIP == 0) { 
			...
			/*
			if (ctrlIFName != NULL) { // if ctrlIFName is not equal to 0  
					// another statemnet to skip lo
				if ((strncmp(recvOnEtherPortIP, ctrlIFName, strlen(ctrlIFName)) != 0) &&  (strncmp(recvOnEtherPortIP, loopbackInterface, strlen(loopbackInterface)) != 0)) {
        			ipReceivedCount++;
					if (enableLogScreen) {
						printf("\n*******************************************************************************");
						printf("\nTEST: IP Packet Received \n");
						printf("\n On port %s", recvOnEtherPortIP);
					}
					...
					memcpy(ipHeadWithPayload, &bufferIP[14], ipPacketSize); //NS ipPakcet  now stored in ipHeadWithPayload
					setInterfaces(); //interfaceList is being set by this function

					unsigned char ipDestTemp[20];
					memset(ipDestTemp, '\0', 20);
					printf("Destination 1 \n"); // comment to test -kriti
					...
					if (packetFwdStatus == 0) { //if we are able to move forward, get inside the loop
						...
						if ((strlen(fwdTierAddr) == strlen(tierAddress))
							...
							if (isFWDFieldsSet() == 0) {
								if (enableLogScreen) {
									printf("TEST: Forwarding IP Packet as EIBP Data Packet (Encapsulation) \n");
		...
		if (flag == 0) {
			...
			ethhead = (unsigned char*)buffer; // this is an Ethernet frame and not just header 
			//printf("\n Size of ethhead %d",sizeof(ethhead));
			if (ethhead != NULL) {
				MPLROtherReceivedCount++;
				uint8_t checkMSGType = (ethhead[14]);
				/*
				Checking for different type of EIBP messages
				0x01 = Hello Message
				...
				*/
				...
				if (strcmp(recvOnEtherPort, "eth0") != 0) {
					...
					if (checkMSGType != MESSAGE_TYPE_CTRL && checkMSGType != MESSAGE_TYPE_DATA && checkMSGType != MESSAGE_TYPE_REQUEST_IP_RESOLVE &&
						...
						printf("\nCURRENT_TIME:%lf\n", conv_time); // time at which the node receives a message and processes it correctly 
						printf("\n Message type is %d",checkMSGType);
						// printf("\nETH_SIZE:%d\n", sizeof(ethhead)*strlen(ethhead));// this was multiplying the frame size with 8 the ethhead size
						printf("\n ETH_SIZE: %d\n", strlen(ethhead)+4);// 4 bytes for the framecheck sequence
...
int main(int argc, char** argv) {
	...
	*/
	...
	//char* filename = "./testLogs.txt"; //The filename to which logs is being saved. NOT BEING USED
			...
			if (ethhead == NULL) {
				...
				if (checkMSGType == MESSAGE_TYPE_LABELS_AVAILABLE) {
					...
					while (!temp) {
						...
						uint8_t labelLength = strlen(label); // Set tbe labellength here
						memcpy(labelAssignmentPayLoad + cplength, &numLabels, 1);
						cplength++;
...
int generateChildLabel(char* myEtherPort, int childTier, struct labels** labelList) {
	...
	//portTag struct update and linked list 	//tag = 2 : to_child port
...
void encaps_ip_msg(char buffer[2048], unsigned char* ethhead, char recvOnEtherPort[5], struct sockaddr_ll src_addr, int *n) {
	...
	if (packetFwdStatus == 0) {
		...
		if (isDestination == 0) {
			...
			// check for null - if not null, call dataDecapsulation
...
void msg_typ_rqst_ip(char buffer[2048], unsigned char* ethhead, char recvOnEtherPort[5], struct sockaddr_ll src_addr) {
	...
	printf("\n TEST: EIBP Message 12 received : %s \n", recvOnEtherPort);
...
void msg_typ_ip_res(char buffer[2048], unsigned char* ethhead, char recvOnEtherPort[5], struct sockaddr_ll src_addr, int *n) {
	printf("\n TEST: EIBP Message 13 received : %s \n", recvOnEtherPort);
	...
	int lengthIndex = 16;
	// check my tier - if I am not tier 1, skip some steps 
	//Destination reading
	int lengthOfMytier = 0;
	lengthOfMytier = (ethhead[lengthIndex]);
